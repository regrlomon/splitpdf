...
parameters = {
    ...
    "temperature": 0.01,
    "num_beams": 1,
    # >1可能会报错，"probability tensor contains either `inf`, `nan` or element < 0"； 即使remove_invalid_values=True也不能解决
    "do_sample": False,
    "top_p": 0.7,
...
class Elembbox(object):
    ...
    right = -1
    bottom = -1
    margin = 4  # for header text above table
    ...
    def __str__(self):
        return """<span style="position:absolute; border: red 1px solid; left:{}px; top:{}px; width:{}px; height:{}px;"></span>""".format(
    ...
    def is_overlap(self, other):
        ...
        def is_pt_in_bbox(x, y, bbox):
            return bbox.left <= x <= bbox.right and \
                   bbox.top - bbox.margin <= y <= bbox.bottom + bbox.margin
        ...
        # 如果两个box间中存在一个角的(x,y)在对方box内，那就说明存在重合的部分
    ...
    def link_horizontal_lines(self, other):
        ...
        if self.top == other.top:
            if self.left < other.left and other.left - 1 <= self.right < other.right:
                # self.left < other.left <=self.right  < other.right
                return True, Elembbox(self.left, self.top, other.right - self.left, self.height)
            elif other.left < self.left and self.left - 1 <= other.right < self.right:
    ...
    def link_vertical_lines(self, other):
        ...
        if self.left == self.left:
            if self.top < other.top and other.top - 1 <= self.bottom < other.bottom:
                # self.top < other.top <=self.bottom  < other.bottom
                return True, Elembbox(self.left, self.top, self.width, other.bottom - self.top)
            elif other.top < self.top and self.top - 1 <= other.bottom < self.bottom:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_file', type=str, default='./GPS.pdf', help='input file')
    ...
    out_f = open(f_name, 'w', encoding='utf-8')
    snippet_arr = []
